...
main() {
	...
	cudaMemcpy(d_1, h_1.data(), bytes, cudaMemcpyHostToDevice);
	cudaMemcpy(d_2, h_2.data(), bytes, cudaMemcpyHostToDevice);
	baseLine << <blocks, threads >> > (d_1, d_2, d_r1, N);
	cudaMemcpy(h_r1.data(), d_r1, bytes, cudaMemcpyDeviceToHost);

	cudaMemcpy(d_3, h_3.data(), bytes, cudaMemcpyHostToDevice);
	cudaMemcpy(d_4, h_4.data(), bytes, cudaMemcpyHostToDevice);
	baseLine << <blocks, threads >> > (d_3, d_4, d_r2, N);
	cudaMemcpy(h_r2.data(), d_r2, bytes, cudaMemcpyDeviceToHost);

	cudaMemcpy(d_5, h_5.data(), bytes, cudaMemcpyHostToDevice);
	cudaMemcpy(d_6, h_6.data(), bytes, cudaMemcpyHostToDevice);
	baseLine << <blocks, threads >> > (d_5, d_6, d_r3, N);
	cudaMemcpy(h_r3.data(), d_r3, bytes, cudaMemcpyDeviceToHost);
