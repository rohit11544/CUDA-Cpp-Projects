...
int main() {
	...
	// kernel 1 launch
	VectorAdd << < Grid_Size, Thread_Size >> > (d_1, d_2, d_r1, N);
	...
	cudaMemcpy(d_4, h_4, bytes, cudaMemcpyHostToDevice);
	// kernel 2 launch
	VectorAdd << < Grid_Size, Thread_Size >> > (d_3, d_4, d_r2, N);
	// copying result from device to host 
	cudaMemcpy(h_r2, d_r2, bytes, cudaMemcpyDeviceToHost);

	// copying arrays from host to device 
	cudaMemcpy(d_5, h_5, bytes, cudaMemcpyHostToDevice);
	cudaMemcpy(d_6, h_6, bytes, cudaMemcpyHostToDevice);
	// kernel 3 launch
	VectorAdd << < Grid_Size, Thread_Size >> > (d_5, d_6, d_r3, N);
	// copying result from device to host 
	cudaMemcpy(h_r3, d_r3, bytes, cudaMemcpyDeviceToHost);
